...
the wave speed $c_0=\omega_0/k_0$, while the other factors give us the envelope
of the wave. Now, however, it propagates not with the wave speed $c_0$ but with
the __group velocity__ $\alpha$ where
$$
\alpha=\frac{d\omega}{dk}(k_0).
$$
In general, the group velocity is different from the wave speed (it is equal to
the wave speed only if there is no dispersion). The shape of the envelope will
still remain the same. However, if we kept a quadratic term in the Taylor
expansion of $\omega(k)$, it would lead to some changes in the shape of the wave
packet, namely: the width of the Gaussian envelope  will increase with time, as
you will discover in a homework problem.
...
A wave packet with wave numbers localised around $k$, as described in lecture
[-@sec-6], moves with the group velocity
$$
