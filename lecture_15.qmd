...
far from the aerofoil and the circulation around the aerofoil.
...
Blasius theorem gives the following expression for the force $\underline{F}$ per unit length on an aerofoil in an incompressible,
...
In this subsection we will derive this expression. For this purpose we first consider the force $d\underline{F}$ on an infinitesimal segment
 of length $dl$
of the cross section of the aerofoil. In terms of the angle $\theta$ that it is indicated in @fig-segment we have
$$
F_x=p\cos\theta dl,~~~F_y=-p\sin\theta dl.
$$ {#eq-15.2}


![Force acting on an infinitesimal segment.](aerofoil_segment.png){width=90% #fig-segment}
...
$$ {#eq-15.11}
(You can find a discussion of Laurent expansions in Definition 6.5 of your
...
We currently have a flow around a cylindrical aerofoil. We want the flow around a less symmetrical shape that is rounded at the front but has a tip
...
Let $A$ be the cross section of a cylindrical aerofoil, i.e., a closed disc, with radius $R$, centred at $z=-\lambda$. Let $\tilde{A}$ be the desired cross section of the aerofoil.
Then $D=\mathbb{C}\setminus A$ and $\tilde{D}=\mathbb{C}\setminus \tilde{A}$ are the regions outside the aerofoils. 
...
$$ {#eq-15.23}
The higher-order terms can be neglected of infinitesimal $\delta z$. For the
...
Let's now consider a second infinitesimal line element from $z_0$ to $z_0+\delta z_2$, 
which gets mapped into a line from $\tilde{z}_0$ to
...
$$ {#eq-15.33}
By calculating the contour integral over a contour surrounding the aerofoil  using Cauchy's residue theorem we find
