...
$$ {#eq-rectangular1}
subject to fixed (Dirichlet) boundary condition
$$
...
and a function of two variables, $x$ and $y$, on the right side. It can be
satisfied for all $x$, $y$ and $t$ only if both sides are equal to a constant.
As in section @sec-4.1, we choose this constant to be negative and (for
...
be equal to a constant, which we choose to be $-\nu^2$ (for some real $\nu$).
Also, setting $\mu$ so that $\nu^2+\mu^2=k^2$ gives us the equations
$$
...
$(n,m)=(2,2)$ and $(n,m)=(3,2)$ are shown in @fig-rectangular_membrane. Any
...
$$
z(x,y,t)=\sum_{n=1}^\infty\sum_{m=1}^\infty z_{n,m}.
$$ {#eq-8.17}
