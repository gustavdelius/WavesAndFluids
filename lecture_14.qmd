...
the first picture in @fig-magnus. We'll see in the next
...
is proportional to $1/z$ becomes irrelevant and $w(z)\approx az$. We have already seen in @exm-straight
...
it, as depicted in the second flow in @fig-magnus. ^[Of course in a perfectly inviscid fluid the cylinder would not be able to drag the fluid along with it due to the absence
...
$$
This is similar to the circular flow from @exm-10_1 just with an extra factor of $-a/r^2$, so this flow too
...
the __line vortex flow__, in spite of its zero vorticity.
...
For a two-dimensional flow with a complex potential $w$, the line integral for the circulation $\Gamma$ in @eq-circulation  can be written as a contour
...
unfortunately it is often adopted without thinking, and I did so in this lecture. Take a look for example at @eq-wl :
$$
w(z)=a\left(z+\frac{R^2}{z}\right)+ib\log z.
$$
