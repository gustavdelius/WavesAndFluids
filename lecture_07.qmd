...
![A membrane stretched in the $x$--$y$ plane and vibrating in the $z$ direction.](membrane.png){#fig-membrane}
...
$$ {#eq-7.13}
(as $\underline{n}$ is a unit vector, i.e. $n_x^2 + n_y^2=1$).
...
$$ {#eq-plane2}
where $\underline{k}=k \, \underline{n}$ is called the _wave vector_. The dispersion relation follows from
@eq-plane2: $\omega(\underline{k})=c \, \vert \underline{k}\vert$.
...
the energy in an n-dimensional region $R\subset\mathbb{R}^n$ is
$$\begin{split}
