...
{{< video <iframe src="https://york.cloud.panopto.eu/Panopto/Pages/Embed.aspx?id=1bc7a507-41c3-4d23-8580-af7901248b54&autoplay=false&offerviewer=true&showtitle=true&showbrand=false&captions=false&interactivity=all" height="405" width="720" style="border: 1px solid #464646;" allowfullscreen allow="autoplay"></iframe> >}}
...
\partial_t^2 z - c^2 \nabla^2 z =0.
$$ {#eq-9.3}
It is well-known (see, e.g., Theorem 3.43 in the Calculus notes) that the
...
We impose the \emph{periodicity} condition
$$
...
$k_{nm}$. We have
$$
R(a)=0 \quad \Rightarrow \quad J_n(k\,a)=0  \quad \Rightarrow \quad k\,a=k_{nm}   \quad \Rightarrow \quad k=\frac{k_{nm}}{a}
$$
for $m\in\mathbb{N}$. 
...
\left(A_{nm}\sin(n\theta)+\cos(n\theta)\right)\\
&\qquad\cdot \left(F_{nm} \, \sin(k_{nm} c t) + G_{nm} \, \cos(k_{nm} c t)\right)
\end{split}$${#eq-circular14}
where $n\in\mathbb{N}\cup\{0\}$, $m\in\mathbb{N}$ and $A_{nm}$, $F_{nm}$ and
$G_{nm}$ are arbitrary real constants. Solutions in the form @eq-circular14 are
...
$$ {#eq-circular15}
Note that constants $A_{nm}$, $F_{nm}$ and $G_{nm}$ (in the formula for $Z_{nm}$
