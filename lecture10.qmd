...
Fluids are considered as continuous media, i.e. any small volume element of a
fluid (a `fluid particle') contains a huge number of molecules. Instead of
...
$\underline{u}(\underline{x},t)$ at time $t$. Then it must satisfy the vector ODE
$$
...
So in Example \ref{ex2} the ODEs for the streamlines are
$$
...
$$ {#eq-10.13}
Note that this is different from the pathlines that we determined for @exm-10.2
...
the rate of change of $f$ at a fixed fluid particle as it moves with the fluid)?
...
&=\partial_t f(\underline{x},t)+\underline{u}\cdot\nabla f(\underline{x},t). 
\end{split}$$ {#eq-10.14}
Thus, we have
$$
\frac{Df}{Dt} = \partial_t f +\underline{u}\cdot\nabla f.
$$ {#eq-10.15}
...
$\underline{u}(\underline{x}(t),t)$. To find acceleration of this fluid particle, $\underline{a}(\underline{x}(t),t)$,
we need to differentiate the velocity with respect to time. Thus
$$
\underline{a}(\underline{x},t) =\frac{d\underline{u}(\underline{x}(t),t)}{dt}= \frac{D \underline{u}}{Dt}
=\partial_t \underline{u} + (\underline{u}\cdot\nabla) \underline{u} .
$$ {#eq-10.17}

If we calculate the acceleration in Example @exm-10.2 where
$\underline{u}=(x,t,0)$ we get the acceleration
$$\begin{split}
\underline{a}(\underline{x},t) &=\frac{D \underline{u}}{Dt}=\partial_t \underline{u} + x\partial_x \underline{u} +t\partial_y \underline{u} +0\partial_y \underline{u} \\
&=(0,1,0)+x(1,0,0)=(x,1,0) .
\end{split}$$ {#eq-10.18}
...
Gauss-Ostrogradsky theorem) of Vector Calculus. If $V$ is a region in
...
$$
\int\limits_{V} \partial_t \rho \, dV = -\int\limits_{V} \nabla\cdot (\rho \, \underline{u}) \, dV,
$$ {#eq-10.23}
which we can rewrite as
$$
\int\limits_{V} \left[\partial_t \rho + \nabla\cdot (\rho \, \underline{u})\right]\, dV = 0.
$$ {#eq-10.24}
The last integral must be zero for any volume $V$. This is possible only if the
integrand is zero. So, we conclude that
$$
\partial_t \rho + \nabla\cdot (\rho \, \underline{u}) =0 .
$$ {#eq-conserv_mass3}
@eq-conserv_mass3 is a conservation equation for the mass, which is also known
as the _continuity equation_.

Note that since $\nabla\cdot (\rho \, \underline{u})=
\underline{u}\cdot\nabla\rho + \rho \, \nabla\cdot\underline{u}$, the equation
of continuity can be written as
$$
\partial_t \rho + \underline{u}\cdot\nabla\rho + \rho \, \nabla\cdot\underline{u} =0
$$ {#eq-conserv_mass4a}
or
$$
\frac{D\rho}{Dt} + \rho\, \nabla\cdot\underline{u} =0 .
$$ {#eq-conserv_mass4b}
It the density of each fluid element remain constant as the fluid moves, i.e.
$$
\frac{D\rho}{Dt} =0 ,
$$ {#eq-conserv_mass5}
then it follows from @eq-conserv_mass4 that
$$
\nabla\cdot\underline{u} =0 .
$$ {#eq-conserv_mass6}
In this case, the fluid is called _incompressible_ and @eq-conserv_mass6 is
known as the _incompressibility condition_. [A vector field $\underline{u}$
satisfying $\nabla\cdot\underline{u}$ is known as a _solenoidal vector field_ or
a _divergence-free vector field_.]{.aside}
